...
| 序号 | 问题需求                                                     | 对应功能                                                     |
| ---- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 1    | 希望减少发布时间统计时手动设置时间区间选项的精力和时间成本   | 程序自动提供时间量子概念（按小时粒度）的可选时间选项         |
| 2    | 希望能够一眼总览整体的时间统计情况，迅速找出参与统计者可用时间的重叠 | 时间统计总览页面的可视化colorbar显示（时间块颜色越深表示在该时间段有空的人越多） |
| 3    | 希望能够快速确定具体时间段的参与人员                         | 时间统计详情页面：带colorbar的滑动时间条和带索引的具体时间参与者列表展示 |
| 4    | 希望能够直观、惊喜的表达自己的空余时间，充分利用碎片时间     | 匹配时间轴的时间量子概念（按小时粒度）的可选时间选项，支持允许参与者保存时间统计查看统计结果 |
| 5    | 希望能够快捷分享表单，方便填写                               | 支持将时间统计表单快速分享到微信聊天                         |
| 6    | 希望能够对时间统计的要求和情况等信息进行定制化               | 新建表单时要求用户填写标题和类别，供统计发布者说明情况和表达约束 |
| 7    | 希望能够对日历上不连续的日期进行时间统计                     | 新建表单时允许用户随机选择日期开展统计（目前最多允许统计3天） |
| 8    | 希望能够有序地管理不同的时间统计                             | 包含统计分类的二级设计，支持分类的新建和分类名编辑，支持表单新建时分类和对移动已有的表单 |
| 9    | 希望能够快速复用先前的时间统计                               | 支持时间统计表单的拷贝                                       |
| 10   | 希望能够快速根据昵称辨识参与统计的人员                       | 参与人员展示和时间统计详情页面均支持带首字母的具体时间参与者列表展示，同时支持昵称的修改 |
...
+ components
  ...
  + Index_list:
  + Peopleshowlist
  + StatColorBar：可视化参与人数渐变显示彩条组件
  + TimeBar_Stat：
  + Timeselect：
  + TimeSelectionBar： 可供选择的单日时间量子条组件
  + TitleAndIntro：表单标题及简介组件
  + UseAlphabet：
  + wx_calendar：第三方“小历同学”日历组件
...
+ Pages
  ...
  + TeamMain： 时间统计页，首页就是各个表单
    * 调用Formlist组件
      * 调用Formunit组件
    
    * TimeMain：表单具体页面，一些设置和跳转处
    
    * （发布逻辑）
    * Form：新建表单明确表单属性页
      * 调用wx_calendar组件
    
    * TimePublish：新建表单明确统计时间段页
    * （统计结果）
    * Stat：统计结果总览页面
      * StatDetail：
